code
proc G_Bounce 104 12
file "..\..\..\..\src/game/g_physics.c"
line 32
;1:/*
;2:===========================================================================
...
;30:================
;31: */
;32:static void G_Bounce(gentity_t *ent, trace_t *trace) {
line 37
;33:  vec3_t velocity;
;34:  float dot;
;35:  int hitTime;
;36:  float minNormal;
;37:  qboolean invert = qfalse;
ADDRLP4 20
CNSTI4 0
ASGNI4
line 40
...
line 45
;44:
;45:  if (ent->s.eType == ET_BUILDABLE) {
ADDRFP4 0
INDIRP4
CNSTI4 4
ADDP4
...
NEI4 $92
line 46
;46:    minNormal = BG_FindMinNormalForBuildable(ent->s.modelindex);
ADDRFP4 0
INDIRP4
CNSTI4 160
ADDP4
...
ASGNF4
line 47
;47:    invert = BG_FindInvertNormalForBuildable(ent->s.modelindex);
ADDRFP4 0
INDIRP4
CNSTI4 160
ADDP4
...
line 52
;50:
;51:  // cut the velocity to keep from bouncing forever
;52:  if ((trace->plane.normal[ 2 ] >= minNormal ||
ADDRLP4 32
ADDRFP4 4
INDIRP4
CNSTI4 32
...
NEI4 $94
line 55
;53:          (invert && trace->plane.normal[ 2 ] <= -minNormal)) &&
;54:          trace->entityNum == ENTITYNUM_WORLD)
;55:    VectorScale(ent->s.pos.trDelta, ent->physicsBounce, ent->s.pos.trDelta);
ADDRLP4 40
ADDRFP4 0
INDIRP4
ASGNP4
...
line 57
;56:  else
;57:    VectorScale(ent->s.pos.trDelta, 0.3f, ent->s.pos.trDelta);
ADDRLP4 64
ADDRFP4 0
INDIRP4
CNSTI4 36
...
line 59
;58:
;59:  if (VectorLength(ent->s.pos.trDelta) < 10) {
ADDRFP4 0
INDIRP4
CNSTI4 36
ADDP4
...
GEF4 $97
line 60
;60:    VectorMA(trace->endpos, 0.5f, trace->plane.normal, trace->endpos); // make sure it is off ground
ADDRLP4 80
ADDRFP4 4
INDIRP4
ASGNP4
...
pop
line 62
;62:    ent->s.groundEntityNum = trace->entityNum;
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
ASGNI4
line 63
;63:    VectorCopy(trace->plane.normal, ent->s.origin2);
ADDRFP4 0
INDIRP4
CNSTI4 104
ADDP4
...
ASGNB 12
line 64
;64:    VectorSet(ent->s.pos.trDelta, 0.0f, 0.0f, 0.0f);
ADDRFP4 0
INDIRP4
CNSTI4 36
ADDP4
...
;66:  }
;67:
;68:  VectorCopy(ent->r.currentOrigin, ent->s.pos.trBase);
ADDRLP4 80
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 69
;69:  VectorAdd(ent->r.currentOrigin, trace->plane.normal, ent->r.currentOrigin);
ADDRLP4 84
ADDRFP4 0
INDIRP4
CNSTI4 488
...
;71:}
LABELV $84
endproc G_Bounce 104 12
export G_Physics
proc G_Physics 96 28
line 81
;72:
;73:#define PHYSICS_TIME 200
;74:
;75:/*
;76:================
;77:G_Physics
...
;79:================
;80: */
;81:void G_Physics(gentity_t *ent, int msec) {
line 88
;82:  vec3_t origin;
;83:  trace_t tr;
;84:  int contents;
;85:  int mask;
;86:
;87:  // if groundentity has been set to -1, it may have been pushed off an edge
;88:  if (ent->s.groundEntityNum == -1) {
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
NEI4 $101
line 89
;89:    if (ent->s.eType == ET_BUILDABLE) {
ADDRFP4 0
INDIRP4
CNSTI4 4
ADDP4
...
NEI4 $103
line 90
;90:      if (ent->s.pos.trType != BG_FindTrajectoryForBuildable(ent->s.modelindex)) {
ADDRLP4 80
ADDRFP4 0
INDIRP4
ASGNP4
...
EQI4 $104
line 91
;91:        ent->s.pos.trType = BG_FindTrajectoryForBuildable(ent->s.modelindex);
ADDRLP4 88
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNI4
line 92
;92:        ent->s.pos.trTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 16
ADDP4
...
;93:      }
line 94
;94:    } else if (ent->s.pos.trType != TR_GRAVITY) {
ADDRGP4 $104
JUMPV
LABELV $103
ADDRFP4 0
...
EQI4 $108
line 95
;95:      ent->s.pos.trType = TR_GRAVITY;
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
ASGNI4
line 96
;96:      ent->s.pos.trTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 16
ADDP4
...
line 101
;99:
;100:  // trace a line from the previous position to the current position
;101:  if (ent->clipmask)
ADDRFP4 0
INDIRP4
CNSTI4 572
...
line 104
;103:  else
;104:    mask = MASK_PLAYERSOLID & ~CONTENTS_BODY; //MASK_SOLID;
ADDRLP4 72
CNSTI4 65537
ASGNI4
LABELV $112
line 106
;105:
;106:  if (ent->s.pos.trType == TR_STATIONARY) {
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
line 111
;109:
;110:    //check floor infrequently
;111:    if (ent->nextPhysicsTime < level.time) {
ADDRFP4 0
INDIRP4
CNSTI4 1028
ADDP4
...
GEI4 $100
line 112
;112:      VectorCopy(ent->r.currentOrigin, origin);
ADDRLP4 60
ADDRFP4 0
INDIRP4
CNSTI4 488
...
line 114
;113:
;114:      VectorMA(origin, -2.0f, ent->s.origin2, origin);
ADDRLP4 80
CNSTF4 3221225472
ASGNF4
ADDRLP4 84
...
line 116
;115:
;116:      trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->s.number, mask);
ADDRLP4 0
ARGP4
ADDRLP4 88
ADDRFP4 0
...
line 121
;120:
;121:      ent->nextPhysicsTime = level.time + PHYSICS_TIME;
ADDRFP4 0
INDIRP4
CNSTI4 1028
ADDP4
...
;128:  BG_EvaluateTrajectory(&ent->s.pos, level.time, origin);
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
line 130
;129:
;130:  trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->s.number, mask);
ADDRLP4 0
ARGP4
ADDRLP4 80
ADDRFP4 0
...
line 132
;131:
;132:  VectorCopy(tr.endpos, ent->r.currentOrigin);
ADDRFP4 0
INDIRP4
CNSTI4 488
ADDP4
...
line 137
;136:
;137:  trap_LinkEntity(ent); // FIXME: avoid this for stationary?
ADDRFP4 0
INDIRP4
ARGP4
...
line 146
;144:
;145:  // if it is in a nodrop volume, remove it
;146:  contents = trap_PointContents(ent->r.currentOrigin, -1);
ADDRFP4 0
INDIRP4
CNSTI4 488
ADDP4
...
ASGNI4
line 147
;147:  if (contents & CONTENTS_NODROP) {
ADDRLP4 76
INDIRI4
CVIU4 4
CNSTU4 2147483648
